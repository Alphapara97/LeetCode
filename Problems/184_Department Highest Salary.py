...
def department_highest_salary(employee: pd.DataFrame, department: pd.DataFrame) -> pd.DataFrame:
    #First, we merge the employee and department dataframes using an inner join (default for merge)
    merged_df = employee.merge(department, left_on = 'departmentId', right_on = 'id')

    #Second, we rename the columns and take only the department, employee, and salary columns
